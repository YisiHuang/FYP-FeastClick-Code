...
public class TicketTypeServiceImpl implements TicketTypeService {
    ...
    public Map<String, Object> onShelf(Long ticketTypeId, HttpSession session) {
        ...
        if (ticketTypeMapper.onShelf(ticketTypeId, username) > 0) {
            map.put(SystemConstant.FLAG, true);
            map.put(SystemConstant.MESSAGE, "Coupon type successfully listed");
        }else {
            map.put(SystemConstant.FLAG, false);
            map.put(SystemConstant.MESSAGE, "Coupon type listing failed");
        }
        return map;
    }
    ...
    public Map<String, Object> offShelf(Long ticketTypeId, HttpSession session) {
        ...
        if (ticketTypeMapper.offShelf(ticketTypeId, username) > 0) {
            map.put(SystemConstant.FLAG, true);
            map.put(SystemConstant.MESSAGE, "Coupon type removed successfully");
        }else {
            map.put(SystemConstant.FLAG, false);
            map.put(SystemConstant.MESSAGE, "Failed to remove coupon type");
        }
        return map;
    }
    ...
    public Map<String, Object> deleteTicketType(Long ticketTypeId) {
        ...
        if (ticketTypeMapper.deleteTicketType(ticketTypeId) > 0) {
            map.put(SystemConstant.FLAG, true);
            map.put(SystemConstant.MESSAGE, "The coupon type was deleted successfully");
        }else {
            map.put(SystemConstant.FLAG, false);
            map.put(SystemConstant.MESSAGE, "Failed to delete the coupon type, some users still have received the coupon and have not used it");
        }
        return map;
    }
