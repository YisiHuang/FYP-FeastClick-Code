...
public class ShopcartServiceImpl implements ShopcartService {
    ...
    public Map<String, Object> addShopcart(Long skuId, Integer numCount, HttpSession session) {
        ...
        if (record==null) {
            ...
            if (shopcartMapper.addShopcart(shopcart) > 0) {
                ...
                map.put(SystemConstant.MESSAGE, "Dish has been added to the cart!");
            }else {
                map.put(SystemConstant.FLAG, false);
                map.put(SystemConstant.MESSAGE, "Failed to add dish to cart");
            }
        }else {
            if (shopcartMapper.addNumCount(record.getShopcartId(), numCount) > 0) {
                map.put(SystemConstant.FLAG, true);
                map.put(SystemConstant.MESSAGE, "The number of dish in the cart has been successfully modified!");
            }else {
                map.put(SystemConstant.FLAG, false);
                map.put(SystemConstant.MESSAGE, "Failed to add dish to cart");
            }
    ...
    public Map<String, Object> deleteShopcart(Long shopcartId) {
        ...
        if (shopcartMapper.deleteShopcart(shopcartId) > 0) {
            map.put(SystemConstant.FLAG, true);
            map.put(SystemConstant.MESSAGE, "The dish has been successfully removed from your cart");
        }else {
            map.put(SystemConstant.FLAG, false);
            map.put(SystemConstant.MESSAGE, "Failed to remove dish from cart");
        }
        return map;
    }
